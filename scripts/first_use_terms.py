...
SOURCE_TERMS = {
    ...
    "memory": ("内存", "保存数据和模型状态的存储空间"),
    "batch": ("批次", "一次并行送入模型处理的一组样本"),
    "token": ("词元", "语言模型处理文本时使用的基本离散单位"),
    "sequence": ("序列", "按顺序排列的一串 token"),
    "hidden dimension": ("隐藏维度", "每个 token 隐藏表示的向量宽度"),
    "view": ("视图", "共享底层存储但用不同逻辑形状解释数据的 tensor"),
    "reshape": ("重塑形状", "在元素总数不变时改变 tensor 的逻辑 shape"),
    "permute": ("维度重排", "重新排列 tensor 各维度的顺序"),
    "copy": ("复制", "真正生成一份新的数据副本"),
    "optimizer": ("优化器", "根据梯度更新模型参数的算法或对象"),
    "loss": ("损失", "衡量模型输出与目标之间误差的标量"),
    ...
    "projection": ("投影", "用可学习线性变换把表示映射到新的特征空间"),
    "attention scores": ("注意力分数", "Query 与 Key 匹配后得到的相关性分数"),
    "causal mask": ("因果掩码", "阻止当前位置看到未来 token 的 Attention 掩码"),
    "softmax": ("归一化指数函数", "把一组分数转换成和为 1 的权重"),
    "head": ("注意力头", "Multi-Head Attention 中独立处理一部分特征的分支"),
    "output projection": ("输出投影", "把多个 attention heads 的结果重新映射回模型隐藏维度"),
    "residual connection": ("残差连接", "把子层输入直接加回子层输出的连接"),
    "LayerNorm": ("层归一化", "按特征维对单个 token 表示做归一化"),
    "RMSNorm": ("均方根归一化", "用均方根缩放特征而不减均值的归一化"),
